    ...
    protected function connect($config, $driverClass, $database, $name, $charset = null)
    {
        ...
        if (isset($config['timezone'])) {
            if (method_exists($this->connections[$connectionKey], 'setTimezone')) {
                $this->connections[$connectionKey]->setTimezone($config['timezone']);
            } else {
                $classname = get_class($this->connections[$connectionKey]);
                $message = sprintf(
                    '%s does not implement "setTimezone". This method is needed to support the timezone on connections',
                    $classname
                );
