...
class UnitOfWork implements PropertyListenerInterface
{
    ...
    public function isNew(NotifyPropertyInterface $entity): bool
    {
        ...
        if (isset($this->entitiesShouldBePersisted[$hash]) === true
            && $this->entitiesShouldBePersisted[$hash] === self::STATE_NEW
        ) {
            return true;
        }
        return false;
    ...
    public function pushSave(NotifyPropertyInterface $entity): self
    {
        ...
        $hash = spl_object_hash($entity);
        $this->entitiesShouldBePersisted[$hash] = $state;
    ...
    {
        $hash = spl_object_hash($entity);
        $this->entitiesShouldBePersisted[$hash] = self::STATE_DELETE;
        $this->entities[$entity] = $entity;
    ...
    {
        foreach ($this->entitiesShouldBePersisted as $hash => $state) {
            switch ($state) {
                case self::STATE_MANAGED:
                    $this->processManaged($hash);
                    break;

                case self::STATE_NEW:
                    $this->processNew($hash);
                    break;

                case self::STATE_DELETE:
                    $this->processDelete($hash);
                    break;
            }
        }
     ...
     * @throws QueryException
     */
    protected function processManaged($hash): void
    {
        ...
        $this->metadataRepository->findMetadataForEntity(
            $entity,
            function (Metadata $metadata) use ($entity, $properties, $hash) {
     ...
     * @throws QueryException
     */
    protected function processNew($hash): void
    {
        $entity = $this->getEntity($hash);
        if ($entity === null) {
            return;
        ...
        $this->metadataRepository->findMetadataForEntity(
            $entity,
            function (Metadata $metadata) use ($entity, $hash) {
                $connection = $metadata->getConnection($this->connectionPool);
     ...
     * @throws QueryException
     */
    protected function processDelete($hash): void
    {
        $entity = $this->getEntity($hash);
        $properties = [];
    ...
    private function getEntity(string $hash): ?NotifyPropertyInterface
    {
        foreach ($this->entities->getIterator() as $entity) {
